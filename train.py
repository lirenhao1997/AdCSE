...
class ModelArguments:
    ...
    sym: bool = field(
        default=False,
        metadata={
            "help": "whether to use the symmetric loss"
        }
    )
...
def main():
    ...
    def prepare_features(examples):
        ...
        # tokenizer() 返回 attention_mask, input_ids, token_type_ids
    ...
    # Data collator, 按batch进行sample展开、padding等操作
    @dataclass
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[
            ...
            # 将 encoded input padding 到 max length
            # padding 后的 attention_mask, input_ids, token_type_ids 维度均为 (batch size * num_sent, max_length)
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(model_path=model_path, config=config)
